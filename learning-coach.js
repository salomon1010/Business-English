...
(function(global){
  ...
  function present(item){
    ...
    const scenarioReturn=item.activityType==="professional_simulation"?`<button class="btn btn-g" id="coachScenarioReturn">Choose another interviewer →</button>`:"";
    el.innerHTML=`<section class="coach-modal" role="dialog" aria-modal="true" aria-label="AI Learning Coach summary"><button class="coach-close" aria-label="Close coaching summary">×</button><div class="eyebrow">AI Learning Coach</div><h2>What you did well</h2><p>${global.esc(item.didWell)}</p><div class="coach-summary-grid"><div><b>Communication</b><span>${global.esc(item.communication)}</span></div><div><b>Vocabulary</b><span>${global.esc(item.vocabulary)}</span></div><div><b>Grammar</b><span>${global.esc(item.grammar)}</span></div><div><b>Confidence</b><span>${global.esc(item.confidence)}</span></div><div><b>Professional communication</b><span>${global.esc(item.professional)}</span></div></div><div class="coach-next"><b>Recommended improvement</b><p>${global.esc(item.improvement)}</p><div style="display:flex;gap:10px;flex-wrap:wrap"><button class="btn btn-p" id="coachNext">${global.esc(item.recommendation.title)} →</button>${scenarioReturn}</div></div></section>`;
    document.body.appendChild(el);const close=()=>el.remove();el.querySelector(".coach-close").onclick=close;el.addEventListener("click",e=>{if(e.target===el)close()});el.querySelector("#coachNext").onclick=()=>{close();openMission(item.recommendation)};const returnButton=el.querySelector("#coachScenarioReturn");if(returnButton)returnButton.onclick=()=>{close();if(typeof global.simChooseAnother==="function")global.simChooseAnother();else global.go("simulation")};
  }
