...
async function learner(id, track, viewport) {
  ...
  await ctx.route(u => u.href.startsWith(POLISH), async route => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ reply: "Say the one decision you need from them, and by when.", covered: coachCovered }) });
  });
...
ok("SEE renders Week 9's title, the director's question and the goal, with one action and a way back — and is worth INTRODUCED only",
  /Leadership-style summaries/i.test(see.txt) && /60-second version/i.test(see.txt) && /Don't start at the beginning/i.test(see.txt) && /Step 1 of 7/i.test(see.txt) && see.cta === 1 && see.back && see.state === "INTRODUCED", see.txt.slice(0, 160));
