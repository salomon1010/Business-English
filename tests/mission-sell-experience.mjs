...
async function learner(id, track, viewport) {
  ...
  await ctx.route(u => u.href.startsWith(POLISH), async route => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ reply: "Say the one step you took yourself, and who you did it with.", one: "Say the one step you took yourself, and who you did it with.", covered: coachCovered }) });
  });
...
ok("K1 · A fresh learner's Home shows no V2 card on Home (removed 2026-09-24 — the mission opens from the session page) — Week 10 does not appear prematurely",
...
ok("SEE renders Week 10's title, the hiring manager's question and the goal, with one action and a way back — and is worth INTRODUCED only",
  /Interview & networking communication/i.test(see.txt) && /Tell me about a project you led/i.test(see.txt) && /Don't recite your duties/i.test(see.txt) && /Mission Hear Notice Speak Coach Complete/.test(see.txt) && see.cta === 1 && see.back && see.state === "INTRODUCED", see.txt.slice(0, 160));
