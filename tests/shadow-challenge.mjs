...
const fl = await A.page.evaluate(async () => {
  ...
  r.shadowPick = svPick; r.shadowText = document.querySelector("#svSh .sv-sh-text")?.innerText; r.want2 = svAsset.segments[6].text; r.listGone = !document.getElementById("svTx");
  play(7, 2); r.stillPinned = svPick === 6 && document.querySelector("#svSh .sv-sh-text")?.innerText === r.want2;   // the tick moving on does not swap the card
