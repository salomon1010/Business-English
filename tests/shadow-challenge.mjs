...
ok("Original + Compare: Play line, Loop and a speed group, Speaker / Me / A/B compare buttons", fb1.orig && fb1.ab && await A.page.evaluate(() => document.querySelectorAll("#svCh .sv-ch-speed button").length === 3 && !!document.getElementById("svChLoopBtn")));
...
const vc = await A.page.evaluate(() => { const ws = svCh.fb.weakWords; const c = ws.find(w => w.vocab), f = ws.find(w => !w.vocab); const stars = [...document.querySelectorAll("#svCh .sv-ch-wk")].map(e => ({ w: e.querySelector(".sv-ch-wk-w").innerText.split("\n")[0], star: !!e.querySelector(".sv-ch-wk-star") }));
  let saved = null, has = null, dropped = null; if (c) { svChSaveVocab(c.k); saved = vocHas(c.k); has = !!document.querySelector("#svCh .sv-ch-wk-star.on"); svChSaveVocab(c.k); dropped = !vocHas(c.k); }
  return { c: c && c.text, f: f && f.text, stars, saved, has, dropped, fnStar: f ? stars.find(x => x.w === f.text)?.star : null }; });
ok("Weakest words: a content word offers ⭐ Save to vocabulary (vocPut, lit when saved, tap again removes it); a function word offers no star", (!vc.c || (vc.saved && vc.has && vc.dropped)) && (vc.f == null || vc.fnStar === false), JSON.stringify(vc));
...
ok("Watch is the transcript's fold: arriving in Watch shows the clip row straight under the tabs, the hint and the notes card, and NO transcript list, with a chevron on Watch; tapping Watch again slides the full transcript in — straight under the tabs, already scrolled to the line being spoken, open (no Transcript button), moving with the video — the clip tools after it", lay.foldedBelow >= 3 && !lay.foldedList && lay.foldedClip && lay.foldedClipFirst && lay.foldedNotes && lay.cardWords > 0 && lay.chev && lay.expanded === "false" && lay.openedBelow >= 4 && lay.openedFirst && !!lay.openedList && lay.openedLines > 5 && !lay.openedFoldBtn && lay.openedScrolled && lay.openedLineIn && lay.moves && lay.expanded2 === "true", JSON.stringify({ foldedBelow: lay.foldedBelow, foldedList: lay.foldedList, foldedClip: lay.foldedClip, foldedClipFirst: lay.foldedClipFirst, foldedNotes: lay.foldedNotes, openedBelow: lay.openedBelow, openedFirst: lay.openedFirst, openedList: lay.openedList, openedLines: lay.openedLines, openedFoldBtn: lay.openedFoldBtn, openedScrolled: lay.openedScrolled, openedLineIn: lay.openedLineIn, moves: lay.moves, cardWords: lay.cardWords, expanded: [lay.expanded, lay.expanded2] }));
...
const fl = await A.page.evaluate(async () => {
  ...
  (svMode === "watch" ? svRender() : svSetMode("watch")); svPick = -1; return r; });
ok("Tapping Watch (fold, then release) never interrupts the follow-along: the card keeps lighting the spoken word with the list folded away, and after the release, and even with no list element at all", fl.before === fl.want1 && fl.afterFold === fl.want2 && fl.listGone && fl.afterRelease === fl.want3 && fl.noList === fl.want4, JSON.stringify(fl));
