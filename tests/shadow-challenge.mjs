...
const lay = await A.page.evaluate(async () => { await shLoad({ vid: "MZAjfsyJa1U", start: 0, end: 0, title: "clip" }, true); await new Promise(r => setTimeout(r, 2500)); shOpenWork(); svPick = -1; svMode = "shadow"; svWatchOpen = false; svSetMode("watch"); svTxOpen = false; svRender(); await new Promise(r => setTimeout(r, 200));
  ...
  const rf = { foldedBelow: below(), foldedList: vis("#svTx"), cardWords: document.querySelectorAll("#svNow .sv-w").length, chev: !!document.querySelector("#svTabs .sv-chev"), expanded: document.querySelector("#svTabs .seg-tab").getAttribute("aria-expanded") };
...
ok("Watch is the fold: arriving in Watch shows only the pinned block (video, two-line card, transport, tabs) with a chevron on Watch; tapping Watch again opens the full transcript — straight under the tabs, already scrolled to the line being spoken, open (no Transcript button), moving with the video — the clip tools after it", lay.foldedBelow === 0 && !lay.foldedList && lay.cardWords > 0 && lay.chev && lay.expanded === "false" && lay.openedBelow >= 4 && lay.openedFirst && !!lay.openedList && lay.openedLines > 5 && !lay.openedFoldBtn && lay.openedScrolled && lay.openedLineIn && lay.moves && lay.expanded2 === "true", JSON.stringify({ foldedBelow: lay.foldedBelow, foldedList: lay.foldedList, openedBelow: lay.openedBelow, openedFirst: lay.openedFirst, openedList: lay.openedList, openedLines: lay.openedLines, openedFoldBtn: lay.openedFoldBtn, openedScrolled: lay.openedScrolled, openedLineIn: lay.openedLineIn, moves: lay.moves, cardWords: lay.cardWords, expanded: [lay.expanded, lay.expanded2] }));
