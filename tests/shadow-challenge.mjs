...
const sh = await A.page.evaluate(() => { const s = svAsset.segments[5]; const card = document.querySelector("#svSh .sv-sh-card"); return { card: !!card, words: document.querySelectorAll("#svSh .sv-sh-w").length, text: card && card.querySelector(".sv-sh-text").innerText, want: s.text, start: shClip.start === s.startMs / 1000, end: shClip.end === s.endMs / 1000, loop: shLooping, note: document.getElementById("shNote").value === s.text, bar: !document.getElementById("svShBar").hidden, barBtns: [...document.querySelectorAll("#svShBar button")].map(b => b.innerText.replace(/\s+/g, " ").trim()), panel: !!document.getElementById("svCh"), noList: !document.getElementById("svTx"), noCtl: getComputedStyle(document.querySelector(".sh-ctl")).display === "none", lang: (S.profile && S.profile.lang) || "en", trBtn: !!document.querySelector("#svSh .sv-sh-rb[onclick*='svShTranslate']") }; });
ok("SHADOW is the paragraph page: the picked paragraph comes whole (word chips + plain text), the clip snaps to it and loops, the notes carry its text for the report, the Speed / Record / Replay bar is at the foot; no transcript list, no transport row, no Challenge panel; no Translate button while the app language is English", sh.card && sh.words === sh.want.split(/\s+/).length && sh.text === sh.want && sh.start && sh.end && sh.loop && sh.note && sh.bar && /Speed/.test(sh.barBtns[0]) && /Record/.test(sh.barBtns[1]) && /Replay/.test(sh.barBtns[2]) && !sh.panel && sh.noList && sh.noCtl && (sh.lang !== "en" || !sh.trBtn), JSON.stringify(sh));
const shn = await A.page.evaluate(() => { svShStep(1); const s6 = svAsset.segments[6]; const r = { pick: svPick, start: shClip.start === s6.startMs / 1000, text: document.querySelector("#svSh .sv-sh-text").innerText === s6.text }; svShStep(-1); r.back = svPick === 5; return r; });
ok("Next / previous paragraph moves the card and the clip together", shn.pick === 6 && shn.start && shn.text && shn.back, JSON.stringify(shn));
const trf = await A.page.evaluate(async () => { S.profile.lang = "fr"; svRender(); const btn = document.querySelector("#svSh .sv-sh-rb[onclick*='svShTranslate']"); const r = { btn: !!btn, code: btn && btn.innerText.trim() }; await svShTranslate(); await new Promise(x => setTimeout(x, 300)); const b = document.getElementById("svShTr"); r.text = b.textContent; r.cls = b.className; S.profile.lang = "en"; svRender(); return r; });
ok("Translate (French app language): the button carries the language code and the paragraph's translation appears under the text (mocked Worker chat route)", trf.btn && trf.code === "FR" && /Traduction/.test(trf.text) && !/err|wait/.test(trf.cls), JSON.stringify(trf));
...
const lay = await A.page.evaluate(async () => { await shLoad({ vid: "MZAjfsyJa1U", start: 0, end: 0, title: "clip" }, true); await new Promise(r => setTimeout(r, 2500)); shOpenWork(); svPick = -1; svMode = "shadow"; svWatchOpen = false; svSetMode("watch"); svTxOpen = false; svRender(); await new Promise(r => setTimeout(r, 200));
  ...
  svSetMode("shadow"); await new Promise(r => setTimeout(r, 200)); r.shClip = vis("#shSeg"); r.card = document.querySelectorAll("#svSh .sv-sh-w").length; r.cardLine = svPick;
...
const fl = await A.page.evaluate(async () => {
  ...
  r.afterFold = play(7, 2); r.want2 = svAsset.segments[7].words[2].text; r.listGone = !document.getElementById("svTx");
  ...
  (svMode === "watch" ? svRender() : svSetMode("watch")); svPick = -1; return r; });
ok("Switching Watch → Shadow → Watch: in Shadow the follow-along card is off the page (the paragraph card replaces it), back in Watch the card lights the spoken word again, and even with no list element at all", fl.before === fl.want1 && !fl.afterFold && fl.listGone && fl.afterRelease === fl.want3 && fl.noList === fl.want4, JSON.stringify(fl));
