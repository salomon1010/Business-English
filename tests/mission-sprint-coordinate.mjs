...
async function learner(id, track, viewport) {
  ...
  await ctx.route(u => u.href.startsWith(POLISH), async route => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ reply: "Name what you are waiting on and who has it before the plan.", covered: coachCovered }) });
  });
...
ok("K1 · A fresh learner's Home offers ONE V2 card and it is Week 1 — Week 8 does not appear prematurely",
...
ok("K3 · Now Home's one card is Week 8, 'Not started', with 'Start the mission' — chosen by the engine's own priority, nothing hardcoded",
  home1.n === 1 && /Week 8/i.test(home1.eyebrow || "") && /Sprint meetings/i.test(home1.title || "") && /not started/i.test(home1.chip || "") && /start the mission/i.test(home1.btn || "") && /say it out loud once/i.test(home1.line || ""), JSON.stringify(home1));
...
ok("SEE renders Week 8's long title, the scrum master's question and the goal, with one action and a way back — and is worth INTRODUCED only",
  /Sprint meetings, blockers & stakeholder coordination/i.test(see.txt) && /where are we/i.test(see.txt) && /Don't just say 'blocked'/i.test(see.txt) && /Step 1 of 7/i.test(see.txt) && see.cta === 1 && see.back && see.state === "INTRODUCED", see.txt.slice(0, 160));
...
ok("Desktop: Home offers the Week 8 card with no horizontal overflow", dkHome.n === 1 && /Week 8/i.test(dkHome.eyebrow || "") && dkHome.o.sw <= dkHome.o.cw, JSON.stringify(dkHome));
