...
async function learner(id, track, viewport) {
  ...
  await ctx.route(u => u.href.startsWith(POLISH), async route => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ reply: "Finish with the one thing you need them to decide.", one: "Finish with the one thing you need them to decide.", covered: coachCovered }) });
  });
