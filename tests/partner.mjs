...
/* ---------- FOUR-ROUND REVIEW: daily topic → four rounds → aggregate → topic-aware analysis → practice → vocabulary → games → progress → next practice ---------- */
{
  ...
  const secs = await A.page.evaluate(() => [...document.querySelectorAll("#v-partner .pp-rv-sec > .eyebrow")].map(e => e.innerText.trim()));
  ok("Report order on screen: voice coach FIRST (owner 2026-09-21) → what you did well → improve → topic mastery → pronunciation → sentence structure → (natural) → vocabulary → rebuilt answer → progress → next practice", /1 · Voice coach/i.test(secs[0]) && /2 · What you did well/i.test(secs[1]) && /3 · What you should improve/i.test(secs[2]) && /4 · Topic mastery/i.test(secs[3]) && /5 · Pronunciation/i.test(secs[4]) && secs.some(s => /8 · Vocabulary to master/i.test(s)) && secs.some(s => /Your answer, rebuilt/i.test(s)) && secs.some(s => /^Progress/i.test(s)) && /Your next practice/i.test(secs[secs.length - 1]), JSON.stringify(secs));
