...
/* ---------- FOUR-ROUND REVIEW: daily topic → four rounds → aggregate → topic-aware analysis → practice → vocabulary → games → progress → next practice ---------- */
{
  ...
  const coaching = await A.page.evaluate(() => { document.getElementById("ppRevCoachBtn").click(); return { on: ppRevCoachOn, txt: document.getElementById("ppRevCoachBtn").innerText }; });
