...
const fab = await A.page.evaluate(() => { /* no manual sync: go("home") itself must have shown it */ const e = document.getElementById("ppFab"); const r = e.getBoundingClientRect(); const nav = document.querySelector(".bnav")?.getBoundingClientRect(); return { on: e.classList.contains("on"), txt: e.innerText.trim(), clearOfNav: !nav || r.bottom <= nav.top, right: r.right <= innerWidth }; });
ok("Home card: in line with someone available → the 'N learner(s) available' card; idle with someone online → green presence counts + 'Find a practice partner'", await A.page.evaluate(() => { const c = document.querySelector(".pp-home"); const inLine = !!c && /available to practise with you/.test(c.innerText); const saved = ppState().lastMe; ppState().lastMe = { ...saved, waiting: null, pair: null, invite: null, live: null }; const idle = ppHomeCardHTML(); ppState().lastMe = saved; return inLine && /learner\(s\) online/.test(idle) && /waiting to practise/.test(idle) && idle.includes("Find a practice partner") && idle.includes('class="pp-green"'); }));
...
ok("Hang up while waiting → the invitation is cancelled, the room is gone, the guest sees no live session", !(await A.page.$(".pp-live-head")) && !(await A.page.evaluate(() => ppLive)) && !(await (await api("eve", "GET", "/me")).json()).live);
