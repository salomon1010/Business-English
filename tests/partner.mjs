...
/* ---------- FOUR-ROUND REVIEW: daily topic → four rounds → aggregate → topic-aware analysis → practice → vocabulary → games → progress → next practice ---------- */
{
  ...
  ok("Report order on screen: what you did well → improve → topic mastery → pronunciation → sentence structure → (natural) → vocabulary → voice coach → rebuilt answer → progress → next practice", /1 · What you did well/i.test(secs[0]) && /2 · What you should improve/i.test(secs[1]) && /3 · Topic mastery/i.test(secs[2]) && /4 · Pronunciation/i.test(secs[3]) && secs.some(s => /7 · Vocabulary to master/i.test(s)) && secs.some(s => /8 · Voice coach/i.test(s)) && secs.some(s => /Your answer, rebuilt/i.test(s)) && secs.some(s => /^Progress/i.test(s)) && /Your next practice/i.test(secs[secs.length - 1]) && secs.findIndex(s => /Vocabulary/i.test(s)) < secs.findIndex(s => /Voice coach/i.test(s)), JSON.stringify(secs));
  ...
  await A.page.waitForFunction(() => !ppRevCoachOn, null, { timeout: 20000 }).catch(() => {});
