...
await A.page.evaluate(() => go("journey")); await sleep(500);   /* owner, 2026-09-19: the button lives on Road map, Shadow and Phrase Lab only */
const fab = await A.page.evaluate(() => { /* no manual sync: go("journey") itself must have shown it */ const e = document.getElementById("ppFab"); if (!e) return { on: false, why: JSON.stringify({ call: !!document.querySelector("#ppCall.on"), shown: ppCallShown, v: cur.v, live: !!ppLive, consented: !!(ppMe && ppMe.consented) }) }; const r = e.getBoundingClientRect(); const nav = document.querySelector(".bnav")?.getBoundingClientRect(); return { on: e.classList.contains("on"), txt: e.innerText.trim().replace(/^\d+\+?\s*/, ""),   /* the online-count badge sits inside the button */ clearOfNav: !nav || r.bottom <= nav.top, right: r.right <= innerWidth }; });
ok("Floating 'Talk to a real person' button on Road map: visible, above the bottom nav", fab.on && fab.txt === (await A.page.evaluate(() => t("pp.fab_disc"))) && fab.clearOfNav && fab.right, JSON.stringify(fab));
ok("Floating button hidden on the partner page and while a call banner is up", await (async () => { await A.page.evaluate(() => go("partner")); await sleep(500); const onPartner = await A.page.evaluate(() => document.getElementById("ppFab").classList.contains("on")); await A.page.evaluate(() => go("journey")); await sleep(400); const withBanner = await A.page.evaluate(() => { ppCallEl().classList.add("on"); ppFabSync(); const on = document.getElementById("ppFab").classList.contains("on"); ppCallEl().classList.remove("on"); ppFabSync(); return on; }); return !onPartner && !withBanner; })());
ok("Floating button on a 375×812 phone does not cover the Road map primary buttons", await (async () => { await A.page.setViewportSize({ width: 375, height: 812 }); await sleep(300); const r = await A.page.evaluate(() => { ppFabSync(); const f = document.getElementById("ppFab").getBoundingClientRect(); const btns = [...document.querySelectorAll("#v-journey .btn-primary, #v-journey .btn-p")].map(b => b.getBoundingClientRect()).filter(b => b.width > 0 && b.bottom > 0 && b.top < innerHeight); const overlap = btns.some(b => !(b.right < f.left || b.left > f.right || b.bottom < f.top || b.top > f.bottom)); return { overlap, w: f.width, h: f.height }; }); await A.page.setViewportSize({ width: 1000, height: 900 }); return !r.overlap && r.h >= 44; })());
/* a learner who has not consented yet (owner's screenshot, 2026-09-20): the button opens the page, joins nothing, shows no error */
{ const N = await learner("newbie", "Newbie Nu", "general-english", false);
  await N.page.evaluate(async () => { await ppRefresh(); go("journey"); }); await sleep(500);
  const before = await N.page.evaluate(() => ({ fab: !!document.getElementById("ppFab") && document.getElementById("ppFab").classList.contains("on"), consented: ppConsented() }));
  let joined = 0; N.page.on("request", r => { if (/\/interest$/.test(r.url()) && r.method() === "POST") joined++; });
  await N.page.click("#ppFab"); await sleep(1200);
  const after = await N.page.evaluate(() => ({ v: cur.v, toast: document.getElementById("toast").classList.contains("show") ? document.getElementById("toast").innerText : "", consentCta: !!document.querySelector("#v-partner .pp-cta"), consentSheet: !!document.querySelector(".pp-consent") }));
  ok("Not yet consented + floating button → the partner page with its consent card, no /interest call, no 'Something went wrong'", before.fab && !before.consented && after.v === "partner" && joined === 0 && !/went wrong/.test(after.toast) && (after.consentCta || after.consentSheet), JSON.stringify({ before, after, joined }));
  await N.ctx.close(); }
...
/* ---------- FOUR-ROUND REVIEW: daily topic → four rounds → aggregate → topic-aware analysis → practice → vocabulary → games → progress → next practice ---------- */
{
  ok("A completed session carries the review card above the decision: 'All four rounds are in' / 'ready', one primary button, private note; nothing before completion", await A.page.evaluate(() => { const c = document.querySelector(".pp-rv-cta"); const d = document.querySelector(".pp-decide"); return !!c && !!d && c.compareDocumentPosition(d) === Node.DOCUMENT_POSITION_FOLLOWING && /rounds are in|review is ready/i.test(c.innerText) && /Private to you/.test(c.innerText) && c.querySelectorAll(".btn-primary").length === 1; }));
  ok("The request carries the DAY'S CURRICULUM as context: week 1, topic 'Tell me about yourself', the task with its four components, the week's phrase bank", await A.page.evaluate(() => { const c = ppRevContext(ppMe.pair); return c.week === 1 && /Tell me about yourself/.test(c.topic) && /current role → core responsibilities → previous experience → current focus/.test(c.task) && c.phrases.length >= 5 && c.phrases.some(p => /responsible for/.test(p.p)) && !!c.objective; }), JSON.stringify(await A.page.evaluate(() => ppRevContext(ppMe.pair))));
  await A.page.click(".pp-rv-cta .btn-primary"); await A.page.waitForSelector(".pp-rv-head", { timeout: 30000 });
  const rv = await A.page.evaluate(() => { const r = ppRevList()[0]; return { n: ppRevList().length, id: r && r.id, topic: r && r.review.topic, seqs: r && r.review.seqs, rounds: r && r.review.rounds.map(x => x.seq), comps: r && r.review.task.components.map(c => c.name + ":" + c.status), ind: r && Object.keys(r.review.indicators), well: r && r.review.well.length, improve: r && r.review.improve.length, script: r && r.review.coach.script.length, polished: r && r.review.answer.polished, next: r && r.review.next, saved: r && Object.keys(r.saved || {}).length }; });
  ok("The review exists once, is topic-anchored ('Tell me about yourself'), covers Alice's rounds 1 and 3 only, judges the curriculum's four components, has five indicators, evidence-based strengths and improvements, a coach script, a polished answer built from her facts, and a next-practice plan", rv.n === 1 && /Tell me about yourself/.test(rv.topic) && rv.seqs.join() === "1,3" && rv.rounds.join() === "1,3" && rv.comps.length === 4 && rv.comps[0].startsWith("current role:") && rv.ind.join() === "pron,grammar,vocab,fluency,task" && rv.well >= 1 && rv.improve >= 1 && rv.script >= 3 && /numbers/.test(rv.polished) && rv.next.vocab.length >= 1 && rv.next.skill, JSON.stringify(rv));
  /* Consolidated hierarchy (owner spec 23 Sep 2026, supersedes the 2026-09-21
     flat order): the primary cards answer first — well → biggest improvement
     → say it better → next practice — and every deeper section is a closed
     fold, with the voice coach kept as the FIRST fold (owner 2026-09-21). */
  const secs = await A.page.evaluate(() => ({
    prim: [...document.querySelectorAll("#v-partner .pp-rv-sec > .eyebrow")].map(e => e.innerText.trim()),
    folds: [...document.querySelectorAll("#v-partner .mv-fold > summary")].map(e => e.innerText.trim()),
    closed: [...document.querySelectorAll("#v-partner .mv-fold")].every(d => !d.open),
    hist: !!document.querySelector("#v-partner .mv-prevlink"),
  }));
  ok("Report hierarchy on screen: what you did well → biggest improvement → say it better → next practice as primary cards; voice coach FIRST of the closed folds, then sentence structure, natural, vocabulary, pronunciation, topic mastery, detailed analysis; a history link at the end", /What you did well/i.test(secs.prim[0]) && /Biggest improvement/i.test(secs.prim[1]) && /Say it better/i.test(secs.prim[2]) && /Your next practice/i.test(secs.prim[secs.prim.length - 1]) && /Voice coach/i.test(secs.folds[0]) && secs.folds.some(s => /Vocabulary to master/i.test(s)) && secs.folds.some(s => /Pronunciation/i.test(s)) && secs.folds.some(s => /Topic mastery/i.test(s)) && /Detailed analysis/i.test(secs.folds[secs.folds.length - 1]) && secs.closed && secs.hist, JSON.stringify(secs));
  /* the interactive checks below reach inside the folds, so open them the
     way a learner would */
  await A.page.evaluate(() => document.querySelectorAll("#v-partner .mv-fold").forEach(d => { d.open = true; }));
  const model = await A.page.evaluate(() => { const m = document.querySelector(".pp-rv-coach .pp-rv-model"); const r = ppRevList()[0].review.coach.model; return { onScreen: !!m, txt: m ? m.innerText : "", struct: r && r.structure, words: r ? r.answer.split(" ").length : 0, facts: r ? /numbers/.test(r.answer) : false, hasSay: !!(m && m.querySelector('[data-rvkey="model"]')) }; });
  ok("Voice coach card carries 'What I was expecting from you': the named structure, a full answer (≥ 20 words) built from Alice's own facts, how it is built, Hear / Slow / Say it", model.onScreen && /What I was expecting from you/i.test(model.txt) && /Present → Past → Future/.test(model.txt) && model.words >= 20 && model.facts && /How it is built/i.test(model.txt) && model.hasSay, JSON.stringify(model).slice(0, 300));
  const scr = await A.page.evaluate(() => ({ about: document.querySelector(".pp-rv-topic")?.innerText, comps: document.querySelectorAll(".pp-rv-comp").length, inds: document.querySelectorAll(".pp-rv-ind").length, chart: !!document.querySelector(".pp-rv-chart"), xs: [...document.querySelectorAll(".pp-rv-ax-x")].map(e => e.textContent), evidence: document.querySelector(".pp-rv-head .pp-fine")?.innerText, ai: /\bAI\b/.test(document.querySelector(".pp-rv-summary")?.innerText || ""), next: document.querySelectorAll(".pp-rv-next .pp-rv-focus li").length, btns: [...document.querySelectorAll(".pp-rv-next .pp-row button")].map(b => b.innerText.trim()), must: document.querySelectorAll(".pp-rv-h4").length }));
  ok("On screen: 'This conversation was about: …', four component rows with a status, five indicator tiles, a rounds chart labelled R1 → R3 → Session, the evidence line, the AI tag on the summary, ≥3 next-practice items, 'Practise with AI' and 'Practise with a human'", /This conversation was about/.test(scr.about) && scr.comps === 4 && scr.inds === 5 && scr.chart && scr.xs.join() === "R1,R3,Session" && /recogniser|listening model|No word scores/.test(scr.evidence) && scr.ai && scr.next >= 3 && /Practise with AI/.test(scr.btns[0]) && /Practise with a human/.test(scr.btns[1]) && scr.must >= 2, JSON.stringify(scr));
  ok("Idempotent on the client too: asking again returns the same review, S.ppRev still holds one entry for the pair", await A.page.evaluate(async () => { const a = ppRevList()[0]; const b = await ppRevRequest(ppMe.pair, true); return b && b.id === a.id && ppRevList().filter(r => r.pairId === ppMe.pair.id).length === 1; }));
  /* privacy: Carla's review is hers, and neither quotes the other */
  const cr = await (await api("carla", "POST", "/pairs/" + (await (await api("carla", "GET", "/me")).json()).pair.id + "/review", { context: { week: 1, topic: "Tell me about yourself", task: "current role → core responsibilities → previous experience → current focus", phrases: [] } })).json();
  const priv = await A.page.evaluate(() => { const r = ppRevList()[0].review; return JSON.stringify([r.fixes.map(f => f.said), r.answer.original, r.well, r.improve, r.pron]); });
  ok("Both learners get independent private reports: Carla's review has her rounds (2, 4) and her words; Alice's never quotes Carla ('See you next week' / 'hardest part?') and Carla's never quotes Alice ('numbers')", cr.review && cr.review.seqs.join() === "2,4" && !/numbers/.test(JSON.stringify([cr.review.fixes.map(f => f.said), cr.review.answer.original, cr.review.pron])) && !/See you next week|hardest part\?/.test(priv) && cr.id !== rv.id, JSON.stringify([cr.review && cr.review.seqs, priv.slice(0, 200)]));
  { const W2 = await learner("wendy2", "Wendy Weld", "welding", false);
    ok("Welding learner: the Worker refuses her on a pair she is not in (403), and the client has no booster card, no review path, no review entries", (await api("wendy", "POST", "/pairs/" + rv.id.replace(/./g, "0").slice(0, 16) + "/review", { context: {} })).status === 403 && await W2.page.evaluate(() => !cvReady() && !document.querySelector('.prac-ex-card[onclick="cvStart()"]') && ppRevList().length === 0 && !ppAvailable()));
    await W2.ctx.close(); }
  /* pronunciation / sentence practice: hear → record → compare → try again, re-scored */
  let assessScore = 86;
  await A.page.route(u => u.href.startsWith("https://be-polish."), async route => { let body = {}; try { body = route.request().postDataJSON() || {}; } catch (e) {}
  ...
  ok("A practice take is graded against its target: 86% → 'Got it', per-word chips, first/best recorded, the day counts as practised", /86%/.test(p1.txt) && /Got it/.test(p1.txt) && p1.tries === 1 && p1.best === 86 && p1.first === 86 && p1.practised, JSON.stringify(p1));
  assessScore = 61;
  await A.page.click(`[data-rvkey="${boxKey}"] .pp-rv-rec`); await sleep(1500); await A.page.click(`[data-rvkey="${boxKey}"] .pp-rv-rec`);
  await A.page.waitForFunction(k => (ppRevList()[0].practice[k] || {}).tries === 2, boxKey, { timeout: 20000 }).catch(() => {});
  const p2 = await A.page.evaluate(k => { const r = ppRevList()[0].practice[k]; return { txt: document.querySelector(`[data-rvkey="${k}"]`).innerText, tries: r.tries, best: r.best, last: r.last }; }, boxKey);
  ok("Try again re-scores: 61% → 'once more', best stays 86, tries 2, a 'Try again' button remains", /61%/.test(p2.txt) && /once more/.test(p2.txt) && p2.tries === 2 && p2.best === 86 && p2.last === 61 && /Try again/.test(p2.txt), JSON.stringify(p2));
  /* voice coach: the script is spoken (provider stubbed away → device voice fallback), counted, and the button returns */
  const coaching = await A.page.evaluate(async () => { document.getElementById("ppRevCoachBtn").click(); await new Promise(r => setTimeout(r, 400)); const l = document.getElementById("ppRevCoachLive"); return { on: ppRevCoachOn, txt: document.getElementById("ppRevCoachBtn").innerText, live: l.className, state: document.getElementById("ppRevCoachState").textContent, now: document.getElementById("ppRevCoachNow").textContent, lit: document.querySelectorAll("#ppRevScript p.now").length }; });
  ok("While the coach speaks: the strip shows moving bars, 'The coach is speaking…' and the sentence being said; that sentence is lit in the script", /\bon\b/.test(coaching.live) && /speaking/.test(coaching.state) && coaching.now.length > 10 && coaching.lit === 1, JSON.stringify(coaching));
  await A.page.waitForFunction(() => !ppRevCoachOn, null, { timeout: 60000 }).catch(() => {});   /* the coach now also reads the expected answer; each line waits for the stubbed provider before the device voice */
  ok("Voice coach: Play → 'Stop' while speaking, counted once on the review, button back to 'Play the coach' when done; the script is readable under 'Read what the coach says'", coaching.on && /Stop/.test(coaching.txt) && await A.page.evaluate(() => ppRevList()[0].coached === 1 && /Play the coach/.test(document.getElementById("ppRevCoachBtn").innerText) && document.querySelectorAll(".pp-rv-script p").length >= 3));
  ok("When the coach finishes: the strip turns green 'The coach has finished', nothing lit, the button is back to Play", await A.page.evaluate(() => /\bdone\b/.test(document.getElementById("ppRevCoachLive").className) && /finished/.test(document.getElementById("ppRevCoachState").textContent) && document.querySelectorAll(".now").length === 0));
  /* save to vocabulary → the app's store, with topic + problem metadata → games */
  const firstSave = await A.page.evaluate(() => { const b = document.querySelector('[data-rvsave^="' + ppRevList()[0].id + ':must:"], [data-rvsave^="' + ppRevList()[0].id + ':upgrade:"]'); b.scrollIntoView(); b.click(); const term = Object.keys(ppRevList()[0].saved)[0]; const v = S.vocab[term]; return { term, v: v && { tk: v.tk, meaning: v.meaning, src: v.src }, def: vocDefStore()[term], btn: b.innerText.trim() }; });
  ok("Save → S.vocab entry tagged general-english with meaning, source Practice Partner, the day's topic and the problem it answers ('missing for today's topic' / 'overused …'); the dictionary store is seeded so the games have a card; button flips to Saved", firstSave.v && firstSave.v.tk.includes("general-english") && firstSave.v.meaning && firstSave.v.src.pp && /Tell me about yourself/.test(firstSave.v.src.topic) && /missing for today|overused/.test(firstSave.v.src.problem) && firstSave.def && firstSave.def.def === firstSave.v.meaning && /Saved/.test(firstSave.btn), JSON.stringify(firstSave));
  await A.page.evaluate(() => ppRevSaveAll(ppRevList()[0].id)); await sleep(300);
  const game = await A.page.evaluate(() => { const ready = pvReady().cv; cvStart(); const g = document.getElementById("pvGame"); const types = _cv.rounds.map(r => r.type); return { ready, open: g && g.classList.contains("show"), title: g && g.querySelector(".pv-title")?.innerText, types, hasUse: types.includes("use"), hasNatural: types.includes("natural"), fixes: cvPool().fixes.length, n: _cv.rounds.length, src: !!g.querySelector(".pp-cv-src, .pv-clue") }; });
  ok("Vocabulary game: saved review items make the Conversation booster ready; it opens with rounds built from them (hear & choose / fill the blank / natural expression / say it / use one of today's expressions)", game.ready && game.open && /Conversation booster/.test(game.title) && game.n >= 3 && game.hasUse && game.types.includes("say") && game.types.includes("hear") && game.hasNatural === (game.fixes > 0), JSON.stringify(game));
  const pick = await A.page.evaluate(() => { const r = _cv.rounds[_cv.i]; if (r.type === "hear" || r.type === "blank") { const before = (S.vocab[r.it.term] || {}).reps || 0; cvPick(r.it.term); return { type: r.type, ok: (S.vocab[r.it.term] || {}).reps === before + 1 }; } if (r.type === "natural") { cvPickNat(1, document.querySelector("#cvOpts .pv-opt")); return { type: r.type, ok: _cv.score === 1 }; } cvSkip(); return { type: r.type, ok: true }; });
  ok("A right answer in the game feeds spaced repetition (reps +1 on a saved word) or the run score", pick.ok, JSON.stringify(pick));
  await A.page.evaluate(() => pvClose());
  /* progress + history: the sessions chart needs two points; the History tab lists the review */
  await A.page.evaluate(() => go("partner", "history")); await sleep(400);
  const hist = await A.page.evaluate(() => ({ h: document.querySelector(".pp-rv-hist")?.innerText || "", rows: document.querySelectorAll(".pp-rv-hist-row").length }));
  ok("History tab: 'Your four-round reviews' lists the session with its topic and indicators", /Your four-round reviews/.test(hist.h) && hist.rows === 1 && /Tell me about yourself/.test(hist.h) && /Topic task/.test(hist.h) === false && /%/.test(hist.h), JSON.stringify(hist).slice(0, 300));
  /* offline: the request is parked and retried when the network is back */
  const off = await A.page.evaluate(async () => { const keep = ppRevList()[0]; S.ppRev = S.ppRev.filter(r => r.id !== keep.id); save(); const orig = ppApi; window._ppApiOrig = orig; ppApi = async () => { throw Object.assign(new Error("network"), { code: "network" }); }; const r = await ppRevRequest(ppMe.pair, true); const pend = ppState().revPending; ppApi = window._ppApiOrig; const back = await ppRevRequest(ppMe.pair, true); return { r, pending: pend && pend.pairId === ppMe.pair.id, cleared: !ppState().revPending, back: back && back.id === keep.id }; });
  ok("Offline: the review request is parked (revPending) without an error page; once the network is back the same review is fetched again and the marker cleared", off.r === null && off.pending && off.back && off.cleared, JSON.stringify(off));
  await A.page.unroute(u => u.href.startsWith("https://be-polish."));
  await A.page.evaluate(() => { ppRevClose(); go("partner"); }); await sleep(1200);
}
...
ok("Guest: the invitation on the partner page says voice call, four rounds, mic only in the call; Join / Not now", (await txt(C.page, ".pp-live-invite")).includes("four timed rounds") && (await txt(C.page, ".pp-live-invite")).includes("Join the call") && (await txt(C.page, ".pp-live-invite")).includes("Not now"));
...
/* since the 2026-09-19 release the production default is ON, so the switch is tested by turning it off explicitly */
await A.page.evaluate(() => { localStorage.setItem("be_flags", JSON.stringify({ practice_partner_enabled: false })); go("practice"); }); await sleep(400);
ok("Flag off → no card on Practice, no Home card, route shows unavailable (the client kill switch)", await A.page.evaluate(async () => { const noCard = !document.querySelector(".pp-entry"); go("partner"); await new Promise(r => setTimeout(r, 300)); return noCard && /temporarily unavailable/.test(document.getElementById("v-partner").innerText); }));
