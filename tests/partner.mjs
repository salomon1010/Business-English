...
ok("While a human is in line the AI coach is STILL offered under the list (owner, 2026-09-19: the learner's choice), and no 'No one is available' card", await A.page.evaluate(() => { ppRender(document.getElementById("v-partner")); const t = document.getElementById("v-partner").innerText; const f = document.querySelector(".pp-fallback"); return !!f && /Or practise with the AI coach/.test(f.innerText) && !/No one is available/.test(t) && document.querySelectorAll(".pp-cand").length > 0 && document.querySelector(".pp-cand").compareDocumentPosition(f) & Node.DOCUMENT_POSITION_FOLLOWING; }), await A.page.evaluate(() => JSON.stringify({ avail: ppMe.waiting && ppMe.waiting.available, cands: Array.isArray(ppCands) ? ppCands.map(c => c.name) : ppCands, txt: document.getElementById("v-partner").innerText.replace(/\s+/g, " ").slice(0, 160) })));
...
ok("A connected learner can look for someone new without ending the partnership: waiting card + connection card still shown (the AI coach card is a standing choice since 8dd93b8)", /looking for your partner|available to practise/i.test(waitingConn) && waitingConn.includes("Your practice partner: Carla") && waitingConn.includes("Stop looking"), waitingConn.slice(0, 400));
