...
ok("K1 · A fresh learner's Home shows no V2 card on Home (removed 2026-09-24 — the mission opens from the session page) — Week 7 does not appear prematurely",
...
ok("SEE renders Week 7's title, the product manager's push and the goal, with one action and a way back — and is worth INTRODUCED only",
  /Disagreeing professionally/i.test(see.txt) && /this Friday/i.test(see.txt) && /Any reason we can't/i.test(see.txt) && /aggressive or weak/i.test(see.txt) && /Mission Hear Notice Speak Coach Complete/.test(see.txt) && see.cta === 1 && see.back && see.state === "INTRODUCED", see.txt.slice(0, 160));
