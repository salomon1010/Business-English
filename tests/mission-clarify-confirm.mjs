...
async function learner(id, track, viewport) {
  ...
  await ctx.route(u => u.href.startsWith(POLISH), async route => {
    ...
    return route.fulfill({ status: 200, contentType: "application/json", body: JSON.stringify({ reply: "Finish by saying what you will do and by when, then ask if that works.", one: "Finish by saying what you will do and by when, then ask if that works.", covered: coachCovered }) });
  });
...
ok("SEE renders Week 4's title, the manager's vague instruction and the goal, with one action and a way back — and is worth INTRODUCED only",
  /Clarifying, asking questions/i.test(see.txt) && /usual metrics/i.test(see.txt) && /Don't just say yes/i.test(see.txt) && /Mission Hear Notice Speak Coach Complete/.test(see.txt) && see.cta === 1 && see.back && see.state === "INTRODUCED", see.txt.slice(0, 160));
