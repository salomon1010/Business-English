/* Executive Polish capture rig — the shots the Phrase Lab flyer is built from.
 *
 ...
 *   NODE_PATH=<where playwright-core is> node scripts/store-art/shoot-polish.js
 *
 * Output: marketing/shots-polish/. The polish result is a live API call, so the
 * wording changes run to run — re-read it out of the console log and keep the
 * flyer copy in step with whatever the committed PNGs actually show.
 */
...
const OUT = path.join(__dirname, "..", "..", "marketing", "shots-polish");
const W = 540, H = 1200, S = 2;                       // -> 1080x2400

/* The sentence is deliberately an ordinary daily standup, because that is the
   meeting this feature is being sold for. */
const RAW = "Yesterday I had a meeting with the technical lead and this is what we did. "
...
const seed = () => {
  ...
  const S = {
    ...
    /* a few phrases already mastered, so the list under the lab reads like a
       returning user rather than an empty shell */
    phMaster: { p0: 1, p1: 1, p4: 1 },
    phExample: { p0: "I currently work as a delivery lead on the payments platform.",
...
(async () => {
  ...
  const fw = () => document.getElementById("fr").contentWindow;
  ...
  await page.evaluate(() => { fw = document.getElementById("fr").contentWindow; fw.go("phrases"); fw.scrollTo(0, 0) });
