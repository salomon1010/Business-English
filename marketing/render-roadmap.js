/* Render roadmap-flyer.src.html (already inlined by build-flyers.py) to the
   shareable PNG + a WhatsApp-friendly JPEG, and report the vertical layout so
   overflow is caught rather than cropped silently.

   node marketing/render-roadmap.js        # needs playwright-core on NODE_PATH
*/
...
(async () => {
  const html = fs.readFileSync(path.join(HERE, 'build/roadmap-flyer.html'), 'utf8');
  ...
  await p.screenshot({ path: path.join(HERE, 'roadmap-flyer@2x.png') });
  await b.close();
})();
