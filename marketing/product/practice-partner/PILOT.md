...
3. Staging Worker (config already in `backend/partner/wrangler.toml [env.staging]`,
   ...
   npx wrangler d1 migrations apply be-partner-staging --remote --env staging   # 0001–0007
...
6. Run `DEVICE_CHECKLIST.md` (60 rows × 2 devices). Record results in the file.
...
| 5 | `npx wrangler d1 migrations apply be-partner --remote` (0001–0007 — all additive) | Cloudflare | |
