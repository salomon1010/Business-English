...
- **Online = askable.** `candidates()` unions the queue with members on the
  track seen in the last 5 minutes (`members.track`, 0008), ranked after
  those in line and carrying `online_now`; `/invite` accepts such a member.
  "Show me candidates" therefore never says "no one" while the strip counts
  someone. Per-candidate lookups (blocks, open pairs, connections,
  cooldowns, exposure) are five set queries, not five per row.
