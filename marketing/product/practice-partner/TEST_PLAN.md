...
## 2. Worker integration — `backend/partner/test/run.mjs` (124 checks)
...
| **AI sessions (4)** | wrong track 403 before anything is recorded · new 201 / repeat 200 no-op · **13th in a day still 201 — AI practice is not rationed** · malformed 400, no auth 401 |
| **No daily practice quota (4)** | five complete sessions on one day (queue → practise → review → decide → `/next` → practise again) with no 429 · rematch then straight back into the queue · 25th queue join still 200 · AI still open after six human sessions |
| **What still bites (3)** | burst keys roll over per minute with no day boundary · only report/block keep a day boundary · the 6th report in a day is refused |
...
## 3. Browser end-to-end — `tests/partner.mjs` (124 checks)
