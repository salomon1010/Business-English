...
(function(global){
  ...
  function prediction(s,t){
    ...
    const remaining=m.next?m.next.at-ready:0;const estimate=m.next?`${Math.max(1,Math.ceil(remaining/rate))} focused week${Math.ceil(remaining/rate)===1?"":"s"} at your recent pace`:"You are at the current roadmap milestone.";
  ...
  function roadmapCard(s){const t=track(),ready=global.CompetencyEngine.readiness(s,t),m=milestone(ready),missing=skills(s,t).slice(0,3),rec=recommendation(s,t),pred=prediction(s,t);return `<section class="card adaptive-roadmap"><div class="eyebrow">Career Readiness Roadmap</div><h2>${global.esc(m.current.title)}</h2><p>${global.esc(m.current.description)}</p><div class="adaptive-grid"><div><span>Current Position</span><b>${global.esc(m.current.title)}</b></div><div><span>Next Milestone</span><b>${global.esc(m.next?m.next.title:"Interview Ready")}</b></div><div><span>Estimated Time</span><b>${global.esc(pred.estimate)}</b></div></div><h3>Skills Missing</h3><div class="adaptive-chips">${missing.map(x=>`<span>${global.esc(x.label)} <b>${x.score}%</b></span>`).join("")}</div><h3>Recommended Activity</h3><p><b>${global.esc(rec.title)}</b> — ${global.esc(rec.reason)}</p><button class="btn btn-p" onclick="AdaptiveLearningEngine.openRecommended()">${global.esc(rec.title)} →</button></section>`}
